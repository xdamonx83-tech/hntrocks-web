...
class ArcadeMatchService
{
    ...
    {
        return DB::transaction(function () use ($match, $actor): ArcadeMatch {
            $match = ArcadeMatch::query()->with('game')->lockForUpdate()->findOrFail($match->id);
            if ($match->status !== ArcadeMatchStatus::WaitingReady) throw new ConflictHttpException('This match is not waiting for players.');
            $player = $match->players()->where('user_id', $actor->id)->lockForUpdate()->first();
            if (! $player) throw new AccessDeniedHttpException;
            if ($player->status === ArcadeMatchPlayerStatus::Ready) return $match->fresh(['game', 'players.user']);
            $player->update(['status' => ArcadeMatchPlayerStatus::Ready, 'ready_at' => now()]);
            $match->version++;
            $required = (int) $match->game->min_players;
            if ($match->players()->where('status', ArcadeMatchPlayerStatus::Ready->value)->count() >= $required) {
                $match->fill(['state' => $this->engines->resolve($match->game)->initialize(), 'status' => ArcadeMatchStatus::Active, 'started_at' => now(), 'current_seat' => 1]);
            }
            $match->save();
