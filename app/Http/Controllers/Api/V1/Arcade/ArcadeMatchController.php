...
class ArcadeMatchController extends Controller
{
    ...
    public function move(Request $request, ArcadeMatch $match): ArcadeMatchResource { $data = $request->validate(['client_move_id' => ['required', 'string', 'max:100'], 'column' => ['required', 'integer']]); return new ArcadeMatchResource($this->moves->move($match, $request->user(), $data['client_move_id'], ['column' => $data['column']])); }
