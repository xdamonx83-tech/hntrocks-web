...
class CommunityCupUpdateController extends Controller
{
    public function __invoke(Request $request, Cup $cup, MediaService $mediaService): RedirectResponse
    {
        ...
        $rulesSettings = $this->extractCupRulesSettings($request, $validated, $cup);
        ...
        $settings['mode'] = (int) ($validated['team_size'] ?? $cup->team_size ?? 1) <= 1
            ? 'solo_leaderboard'
            : 'team_leaderboard';
        $settings['event_key'] = $settings['event_key'] ?? null;
        $settings['scoring'] = $settings['scoring'] ?? 'bounty_first_extract_required';
        $settings['content'] = $contentSettings;

        $validated['settings'] = array_replace_recursive($settings, $rulesSettings);
        ...
        return redirect()
            ->route('cups.show', $cup)
            ->with('status', __('ui.cup_saved_status'));
    }
    ...
    private function extractCupRulesSettings(Request $request, array &$validated, Cup $cup): array
    {
        ...
        $verificationMode = CupOrganizerAccess::sanitizeRequestedVerificationMode(
            $request->user(),
            $validated['verification_mode'] ?? CupOrganizerAccess::verificationMode($cup)
        );
        ...
        $validated['platform'] = collect($allowedPlatforms)->implode(' / ');
        ...
        return [
            ...
            'platform_gate' => ['allowed_platforms' => $allowedPlatforms],
            'submission_limit' => [
