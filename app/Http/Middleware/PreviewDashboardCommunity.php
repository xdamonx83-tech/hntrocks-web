...
class PreviewDashboardCommunity
{
    ...
    private function communityPayload(User $viewer): array
    {
        ...
        $totalMembers = (int) $this->safe(
            fn (): int => User::query()->where('status', 'active')->count(),
            0
        );

        $activeToday = (int) $this->safe(
            fn (): int => User::query()
                ->where('status', 'active')
        ...
        $onlineNow = max(1, $onlineNow);

        $newThisWeek = (int) $this->safe(
            fn (): int => User::query()
                ->where('status', 'active')
        ...
        $openLfgs = (int) $this->safe(
            fn (): int => LfgPost::query()
                ->where('status', 'open')
        ...
        $activeCupTeams = (int) $this->safe(
            fn (): int => CupTeam::query()
                ->where('status', 'active')
    ...
    private function recentActivity(Carbon $now): array
    {
        ...
        $this->safe(function () use ($items): void {
            FeedPost::query()
                ->with('user')
    ...
    private function trendingHashtags(Carbon $since): array
    {
        ...
        $this->safe(function () use ($tags, $since): void {
            FeedPost::query()
                ->where('status', 'published')
                ->where('visibility', '!=', 'private')
                ...
                ->limit(120)
                ->pluck('body')
                ->each(function (?string $text) use ($tags): void {
                    foreach (Hashtag::extract($text) as $tag) {
                        $tags->push($tag);
                    }
                });
        }, null);
        ...
        $this->safe(function () use ($tags, $since): void {
            Moment::query()
                ...
                ->each(function (Moment $moment) use ($tags): void {
                    foreach (array_merge(Hashtag::extract($moment->caption), Hashtag::extract($moment->description)) as $tag) {
                        $tags->push($tag);
                    }
                });
        }, null);
        ...
        $this->safe(function () use ($tags, $since): void {
            LfgPost::query()
                ...
                ->each(function (LfgPost $post) use ($tags): void {
                    foreach (array_merge(Hashtag::extract($post->title), Hashtag::extract($post->body)) as $tag) {
                        $tags->push($tag);
                    }
                });
        }, null);
