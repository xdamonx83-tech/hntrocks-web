@php
    ...
    $sessionMaybe = $teamDetail['sessionMaybe'];
    ...
    $sessionWeekProgress = $sessionsThisWeek > 0 ? min(100, (int) round(($completedSessionsThisWeek / $sessionsThisWeek) * 100)) : 0;
    ...
    $sessionCapacity = $upcomingSession ? max(1, (int) ($upcomingSession->max_participants ?: $activeMembers->count())) : 0;
@endphp
...
@if($team->region || $team->language)<span>{{ collect([$team->region, $team->language])->filter()->implode(' · ') }}</span>@endif
</div>
<p>{{ $teamTagline }}</p>
</div>
...
<article><strong>{{ (int) $team->sessions_count }}</strong><span>{{ $isEnglish ? 'Sessions' : 'Sessions' }}</span></article>
...
@if($team->avatar_path)<img src="{{ $team->avatarUrl() }}" alt="{{ $teamName }}">@else{{ $teamInitials }}@endif
</div>
<div>
...
<form method="post" action="{{ route('teams.join', $team) }}">@csrf<button type="submit">{{ $isEnglish ? 'Request to join' : 'Beitritt anfragen' }}</button></form>
...
<form method="post" action="{{ route('teams.leave', $team) }}" onsubmit="return confirm('{{ $isEnglish ? 'Leave this team?' : 'Dieses Team wirklich verlassen?' }}')">@csrf<button type="submit">{{ $isEnglish ? 'Leave team' : 'Team verlassen' }}</button></form>
@endif
...
<div><dt>{{ $isEnglish ? 'Region' : 'Region' }}</dt><dd>{{ $team->region ?: '—' }}</dd></div>
...
<div><span>{{ Str::upper($teamName) }}</span><h2 id="teamPanelTitle">{{ $isEnglish ? 'Overview' : 'Übersicht' }}</h2></div>
...
@if($canManage)<button data-team-tab="requests" type="button">{{ $isEnglish ? 'Requests' : 'Anfragen' }} <i>{{ $team->pending_count }}</i></button>@endif
</nav>
</header>
...
<header><div><span>{{ $isEnglish ? 'NEXT SESSION' : 'NÄCHSTE SESSION' }}</span><h3>{{ $upcomingSession?->title ?: ($isEnglish ? 'No session planned' : 'Keine Session geplant') }}</h3></div>@if($upcomingSession)<strong>{{ $upcomingSession->starts_at->isToday() ? ($isEnglish ? 'Today' : 'Heute') : $upcomingSession->starts_at->translatedFormat('D, d.m.') }}</strong>@endif</header>
@if($upcomingSession)
<div class="session-time-row">
<div><strong>{{ $upcomingSession->starts_at->format('H:i') }}</strong><span>{{ $isEnglish ? 'Start' : 'Start' }}</span></div>
...
<div class="session-attendees"><div class="team-avatar-stack">@foreach($sessionGoing->take(4) as $response)<img alt="{{ $response->user?->name }}" src="{{ $response->user?->avatarUrl() ?: asset('assets/vikinger/img/default-avatar.svg') }}"/>@endforeach</div><span>{{ $goingCount ? ($isEnglish ? $goingCount.' members confirmed.' : $goingCount.' Mitglieder haben zugesagt.') : ($isEnglish ? 'No confirmations yet.' : 'Noch keine Zusagen.') }}</span></div>
@if($isMember)
<form class="team-session-response" method="post" action="{{ route('teams.sessions.respond', [$team, $upcomingSession]) }}">@csrf
...
<header><div><span>{{ $isEnglish ? 'LATEST TEAM UPDATE' : 'LETZTES TEAM-UPDATE' }}</span><h3>{{ $latestPost ? Str::limit(strip_tags((string) $latestPost->body), 58) : ($isEnglish ? 'No team posts yet' : 'Noch keine Team-Beiträge') }}</h3></div><button data-open-team-tab="posts" type="button">{{ $isEnglish ? 'All posts' : 'Alle Beiträge' }}</button></header>
@if($latestPost)
...
<article>@if($activity['url'])<a href="{{ $activity['url'] }}"><img alt="" src="{{ $activity['avatar'] }}"/></a>@else<img alt="" src="{{ $activity['avatar'] }}"/>@endif<div><strong>{{ $activity['title'] }}</strong><small>{{ $activity['meta'] }}</small></div><span>{{ optional($activity['at'])->diffForHumans() }}</span></article>
@empty
...
@php($contractPercent = min(100, max(0, (int) round(((int) $contract->progress_value / max(1, (int) $contract->target_value)) * 100))))
...
<div class="post-body">
@if($post->body)<div class="team-real-post-copy">{!! \App\Support\FeedTextRenderer::render($post->body) !!}</div>@endif
@if($postMedia)
<div class="team-real-post-media">@if($postMedia->isImage())<img src="{{ $postMedia->url() }}" alt="{{ $postMedia->original_name }}">@elseif($postMedia->isVideo())<video controls preload="metadata"><source src="{{ $postMedia->url() }}" type="{{ $postMedia->mime_type }}"></video>@endif</div>
@endif
@if($postPoll)
<div class="poll">@foreach($postPoll->options as $option)@php($optionVotes = $option->votes->count())<a href="{{ $post->permalink() }}"><span>{{ $option->body }}</span><b>{{ $pollTotal > 0 ? round(($optionVotes / $pollTotal) * 100) : 0 }}%</b></a>@endforeach<small>{{ $pollTotal }} {{ $isEnglish ? 'votes' : 'Stimmen' }}</small></div>
@endif
</div>
...
<article class="team-data-card"><span>{{ $isEnglish ? 'TEAM DATA' : 'TEAMDATEN' }}</span><dl><div><dt>{{ $isEnglish ? 'Founded' : 'Gründung' }}</dt><dd>{{ $team->created_at?->translatedFormat('d. F Y') ?: '—' }}</dd></div><div><dt>{{ $isEnglish ? 'Visibility' : 'Sichtbarkeit' }}</dt><dd>{{ $team->visibilityLabel() }}</dd></div><div><dt>Recruiting</dt><dd>{{ $team->recruitmentLabel() }}</dd></div><div><dt>{{ $isEnglish ? 'Platform' : 'Plattform' }}</dt><dd>{{ $team->platform ?: '—' }}</dd></div><div><dt>{{ $isEnglish ? 'Region' : 'Region' }}</dt><dd>{{ $team->region ?: '—' }}</dd></div><div><dt>{{ $isEnglish ? 'Language' : 'Sprache' }}</dt><dd>{{ $team->language ?: '—' }}</dd></div></dl></article>
</section>
...
@endphp
<article><header><img alt="{{ $user?->name }}" src="{{ $user?->avatarUrl() ?: asset('assets/vikinger/img/default-avatar.svg') }}"/><span>{{ Str::upper($member->roleLabel()) }}</span>@if($user)<a href="{{ route('profile.public', $user) }}"><svg><use href="#i-arrow"></use></svg></a>@endif</header><h3>{{ $user?->name ?: $user?->username }}</h3><p>{{ '@'.($user?->username ?: 'hunter') }} · {{ $online ? ($isEnglish ? 'online' : 'online') : ($user?->last_seen_at?->diffForHumans() ?: ($isEnglish ? 'offline' : 'offline')) }}</p><div class="member-stats"><span><strong>{{ max(1,(int)($user?->level ?? 1)) }}</strong>Level</span><span><strong>{{ (int)($user?->published_posts_count ?? 0) }}</strong>Posts</span><span><strong>{{ number_format((int)($user?->crownWallet?->balance ?? 0),0,',','.') }}</strong>Rocks</span></div><div class="member-tags">@foreach(collect([$profile?->platform,$profile?->region,$profile?->playstyle,$profile?->language])->filter()->take(4) as $tag)<span>{{ $tag }}</span>@endforeach</div><footer><i class="{{ $online ? 'online' : '' }}"></i><span>{{ $online ? ($isEnglish ? 'Online' : 'Online') : $member->roleLabel() }}</span></footer></article>
@empty
...
<div class="team-request-intro"><div><span>{{ $isEnglish ? 'JOIN REQUESTS' : 'BEITRITTSANFRAGEN' }}</span><h3>{{ trans_choice($isEnglish ? ':count open request|:count open requests' : ':count offene Anfrage|:count offene Anfragen', $pendingMembers->count(), ['count'=>$pendingMembers->count()]) }}</h3><p>{{ $isEnglish ? 'Review profile and message before accepting a request.' : 'Prüfe Profil und Nachricht, bevor du eine Anfrage annimmst.' }}</p></div><span class="request-counter">{{ $pendingMembers->count() }} {{ $isEnglish ? 'open' : 'offen' }}</span></div>
<div class="team-request-list" id="teamRequestList">
@forelse($pendingMembers as $requestMember)
@php($requestUser = $requestMember->user)
...
@php($memberOnline = $onlineMembers->contains('user_id',$member->user_id))
<article class="{{ $memberOnline ? '' : 'offline' }}"><img alt="{{ $member->user?->name }}" src="{{ $member->user?->avatarUrl() ?: asset('assets/vikinger/img/default-avatar.svg') }}"/><div><strong>{{ $member->user?->name ?: $member->user?->username }}</strong><small>{{ $member->roleLabel() }} · {{ $memberOnline ? ($isEnglish ? 'online' : 'online') : ($member->user?->last_seen_at?->diffForHumans() ?: 'offline') }}</small></div><i></i></article>
@empty
