...
@php
    ...
    $fieldClass = 'cup-form-input';
    $textareaClass = 'cup-form-input cup-form-textarea';
    $labelClass = 'cup-form-label';
    $helpClass = 'cup-form-help';
    $sectionTitleClass = 'cup-form-section-title';
    $sectionTextClass = 'cup-form-section-text';
    $cardClass = 'cup-form-card';
    $heroImage = $cup->exists && $cup->cover_path ? $cup->coverUrl() : asset('assets/themes/hnt_preview/assets/quick-cup-cover.jpg');
...
<div class="cup-form-shell" id="js-cup-form-shell">
    <div class="cup-form-main">
        <div class="cup-form-panel">
            <div class="cup-form-hero">
                <img src="{{ $heroImage }}" alt="" class="cup-form-hero-image">
                <div class="cup-form-hero-shade"></div>
                <div class="cup-form-hero-content">
                    <div class="cup-form-kicker">
            ...
            <div class="cup-form-body">
                <section class="cup-form-section">
                    <div>
                    ...
                    <div class="cup-form-grid">
                        ...
                        <div>
                            ...
                            <div class="cup-form-check-list">
                                @foreach ($platformOptions as $platformValue => $platformLabel)
                                    <label class="cup-form-check-option hnt-cup-check-option">
                ...
                <section class="cup-form-section">
                    ...
                    <div class="cup-form-grid">
                        ...
                        <div>
                            ...
                            <label class="cup-form-check-option cup-form-check-option--stack hnt-cup-check-option">
                                <input type="hidden" name="require_profile_complete" value="0">
                                <input class="hnt-cup-check-input" type="checkbox" name="require_profile_complete" value="1" @checked($requiresProfileComplete)>
                                <span class="hnt-cup-check-box" aria-hidden="true"></span>
                                <span>
                                    <span class="cup-form-check-title">{{ __('ui.cup_form_require_profile_complete') }}</span>
                                    <span class="{{ $helpClass }}">{{ __('ui.cup_form_require_profile_complete_hint') }}</span>
                                </span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="cup-form-locale-grid">
                        @foreach ([
                            ...
                            <div class="cup-form-locale-card">
                                ...
                                <div class="cup-form-prize-block">
                                    <h4>{{ __('ui.cup_form_prizes_title') }}</h4>
                ...
                <section class="cup-form-section">
                    ...
                    <input class="{{ $fieldClass }}" id="cover" name="cover" type="file" accept="image/*">
                </section>
            </div>
        </div>
    </div>

    <aside class="cup-form-aside">
        <div class="cup-form-aside-stack" uk-sticky="media: 1024; end: #js-cup-form-shell; offset: 80">
            <div class="{{ $cardClass }}">
                <div>
                    <p class="cup-form-side-kicker">{{ __('ui.cup_info_setup') }}</p>
                    <h3>{{ $displayTitle }}</h3>
                    <p>{{ $cup->exists ? __('ui.cup_edit_intro') : __('ui.cup_create_intro') }}</p>
                </div>

                <div class="cup-form-stat-grid">
                    <div class="cup-form-stat">
                        <div>{{ __('ui.cup_table_status') }}</div>
                        <strong>{{ __('ui.cup_status_'.$displayStatus) }}</strong>
                    </div>
                    <div class="cup-form-stat">
                        <div>{{ __('ui.platform') }}</div>
                        <strong>{{ $displayPlatform }}</strong>
                    </div>
                    <div class="cup-form-stat">
                        <div>{{ __('ui.cup_form_team_size') }}</div>
                        <strong>{{ $displayTeamSize }}</strong>
                    </div>
                    <div class="cup-form-stat">
                        <div>{{ __('ui.cup_form_rules_preset') }}</div>
                        <strong>{{ \App\Models\Cup::rulesPresetOptions()[$selectedRulesPreset] ?? $selectedRulesPreset }}</strong>
                    </div>
                </div>

                <div class="cup-form-summary-list">
                    <div><span>{{ __('ui.cup_start') }}</span><strong>{{ $displayStart ?: __('ui.cup_open') }}</strong></div>
                    <div><span>{{ __('ui.cup_end') }}</span><strong>{{ $displayEnd ?: __('ui.cup_open') }}</strong></div>
                    <div><span>{{ __('ui.cup_form_max_submissions_per_participant') }}</span><strong>{{ $maxSubmissionsPerParticipant ?: '∞' }}</strong></div>
                    <div><span>{{ __('ui.cup_form_max_scored_submissions_per_participant') }}</span><strong>{{ $maxScoredSubmissionsPerParticipant ?: '∞' }}</strong></div>
                </div>

                <div class="cup-form-actions">
                    <button class="btn-create" type="submit">{{ $submitLabel }}</button>
                    <a class="cup-icon-btn" href="{{ $backUrl }}" uk-tooltip="title: {{ $backLabel }}; offset: 8">
            ...
            <div class="{{ $cardClass }}">
                <h3>{{ __('ui.cup_form_ruleset_title') }}</h3>
                <div class="cup-form-note-list">
