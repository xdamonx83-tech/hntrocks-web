    ...
    <section class="hnt-login-frame" aria-labelledby="hnt-login-title">
        <section class="hnt-login-panel">
            <header class="hnt-login-header">
                <a class="hnt-login-brand" href="{{ route('home') }}" aria-label="{{ __('hnt_auth.home_label') }}">
                    <img src="{{ $hntLoginLogo }}" alt="" aria-hidden="true">
                    <span>HNT.ROCKS</span>
                </a>

                <div class="hnt-login-header-actions">
                    <nav class="hnt-login-language" aria-label="{{ __('hnt_auth.language_label') }}">
                    ...
                    <a class="hnt-login-close" href="{{ route('home') }}" aria-label="{{ __('hnt_auth.home_label') }}">
            ...
            <div class="hnt-login-center">
                <div class="hnt-login-content">
                    <header class="hnt-login-heading">
                        <span>{{ __('hnt_auth.kicker') }}</span>
                        <h1 id="hnt-login-title">{{ __('hnt_auth.login_heading') }}</h1>
                        <p>{{ __('hnt_auth.login_intro') }}</p>
                    </header>
                    ...
                    <form class="hnt-login-form" method="POST" action="{{ route('login.store') }}" data-hnt-login-form data-loading-label="{{ __('hnt_auth.login_loading') }}">
                        @csrf

                        <div class="hnt-login-field">
                            <label for="login-identity">{{ __('hnt_auth.identity_label') }}</label>
                            <div class="hnt-login-control @error('login') is-invalid @enderror">
                                <input id="login-identity"
                                       ...
                                       autocapitalize="none"
                                       spellcheck="false"
                                       placeholder="{{ __('hnt_auth.identity_placeholder') }}"
                                       required
                                       autofocus
                        ...
                        <div class="hnt-login-field">
                            <label for="login-password">{{ __('hnt_auth.password_label') }}</label>
                            <div class="hnt-login-control hnt-login-password-control @error('password') is-invalid @enderror">
                                <input id="login-password"
                                       name="password"
                                       type="password"
                                       autocomplete="current-password"
                                       placeholder="{{ __('hnt_auth.password_placeholder') }}"
                                       required
                                       @error('password') aria-describedby="login-password-error" @enderror>
                                <button type="button"
                                        data-hnt-password-toggle
                                        data-show-label="{{ __('hnt_auth.show_password') }}"
                                        data-hide-label="{{ __('hnt_auth.hide_password') }}"
                                        aria-label="{{ __('hnt_auth.show_password') }}"
                        ...
                        <div class="hnt-login-options">
                            <label class="hnt-login-check" for="login-remember">
                                ...
                                <b>{{ __('hnt_auth.remember_me') }}</b>
                            </label>

                            <a href="{{ route('password.request') }}">{{ __('hnt_auth.forgot_password') }}</a>
                        </div>

                        <button class="hnt-login-primary" type="submit" data-hnt-login-submit>
                            <span data-hnt-login-submit-label>{{ __('hnt_auth.login_submit') }}</span>
                            <span class="hnt-login-spinner" aria-hidden="true"></span>
                        </button>

                        @if ($hntSocialDefinitions->isNotEmpty())
                            <div class="hnt-login-divider"><span>{{ __('hnt_auth.continue_with') }}</span></div>

                            <div class="hnt-login-social">
                                @foreach ($hntSocialDefinitions as $provider)
                                    <a href="{{ route('social.redirect', ['provider' => $provider['key']]) }}"
                                       aria-label="{{ __('hnt_auth.social_login', ['provider' => $provider['label']]) }}">
            ...
            <footer class="hnt-login-footer">
                <div>
                    <span>{{ __('hnt_auth.no_account') }}</span>
                    <a href="{{ route('register') }}">{{ __('hnt_auth.register_now') }}</a>
                </div>
