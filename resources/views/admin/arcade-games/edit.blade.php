...
@foreach(['name_de'=>'Name DE','name_en'=>'Name EN','badge_de'=>'Badge DE','badge_en'=>'Badge EN','client_engine_key'=>'Client Engine Key','launch_url'=>'Launch URL','sort_order'=>'Sortierung','min_players'=>'Min. Spieler','max_players'=>'Max. Spieler','game_version'=>'Game Version'] as $field=>$label)<label>{{ $label }}<input name="{{ $field }}" value="{{ old($field, $game->$field) }}"></label>@error($field)<div>{{ $message }}</div>@enderror @endforeach
