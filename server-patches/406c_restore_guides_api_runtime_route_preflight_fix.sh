...
cat <<'HEAD'
============================================================
HNT.ROCKS PATCH 406c - GUIDES API RESTORE / ROUTE PREFLIGHT FIX
============================================================
- setzt den 406-Restore erneut direkt von current main auf
- uebernimmt 406a: intent-to-add + korrekten Kernel-Smoke-Heredoc
- uebernimmt 406b: alte Guide-Comment-Web-Routen auf bestehende API-URLs umstellen
- korrigiert jetzt den stillen Abbruch in Schritt [6/14]:
  * artisan route:clear/config:clear werden sichtbar und explizit geprueft
  * route:list stdout/stderr werden bei Fehler ausgegeben statt still im JSON zu verschwinden
  * ungueltiges `php - <arg>` fuer den JSON-Vertragscheck wird durch STDIN + ENV ersetzt
- erst wenn Route-Preflight + Auth-Kernel-Smoke gruen sind: Feature-Commit -> Backup -> Deploy -> Live-Smokes
...
'''
if s.count(old) != 1:
    raise SystemExit(f"406c: Scope-Anker erwartet 1x, gefunden {s.count(old)}x")
...
'''
if s.count(old) != 1:
    raise SystemExit(f"406c: Smoke-Startanker erwartet 1x, gefunden {s.count(old)}x")
s = s.replace(old, new, 1)
...
run_kernel_smoke "$WT" "PREDEPLOY"'''
if s.count(old) != 1:
    raise SystemExit(f"406c: Smoke-Endanker erwartet 1x, gefunden {s.count(old)}x")
s = s.replace(old, new, 1)
...
if s.count(anchor) != 1:
    raise SystemExit(f"406c: Comment-Route-Anker erwartet 1x, gefunden {s.count(anchor)}x")
s = s.replace(anchor, replacement, 1)

# 406c: Schritt 6 darf Artisan-Fehler nicht in einer JSON-Datei verstecken.
...
'''
if s.count(old) != 1:
    raise SystemExit(f"406c: Route-Preflight-Anker erwartet 1x, gefunden {s.count(old)}x")
s = s.replace(old, new, 1)

p.write_text(s, encoding='utf-8')
PY

bash -n "$TMP" || fail "korrigierter 406c-Harness ist syntaktisch ungueltig"
...
grep -Fq 'ROUTE_JSON_PATH="$ROUTE_JSON" php <<' "$TMP" || fail "Route-JSON STDIN-Fix fehlt"
grep -Fq 'route:list STDERR' "$TMP" || fail "sichtbare route:list Fehlerausgabe fehlt"
if grep -Fq 'php - "$ROUTE_JSON"' "$TMP"; then fail "ungueltiger php-minus Aufruf ist noch vorhanden"; fi

echo "406c Harness: OK"
echo
bash "$TMP"
