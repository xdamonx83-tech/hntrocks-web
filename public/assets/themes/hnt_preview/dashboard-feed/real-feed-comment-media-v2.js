/* Robust comment media display + upload bridge for the isolated dashboard preview. */
(() => {
  ...
  style.id = 'real-comment-media-v2-styles';
  style.textContent = `
  ...
  const syncActivePost = (sourceButton = null) => {
    ...
    const postId = sourcePostId || modalPostId;
    ...
    state.trigger = sourceButton
      || article?.querySelector('[data-real-preview-comments]')
      || state.trigger;
  ...
  const ensureMainComposer = () => {
    const current = formState(composer);
  ...
  const mediaMapFromHtml = (html) => {
    ...
    parsed.querySelectorAll('[data-hnt-comment-item]').forEach((comment) => {
      ...
      const media = [...comment.querySelectorAll('.hnt-comment-media-thumb')]
        .map((item) => {
